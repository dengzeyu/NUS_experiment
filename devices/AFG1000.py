#control class taken from https://github.com/asvela/tektronix-func-gen/tree/main
...
class AFG1000():
    def __init__(self, adress = 'COM1'):
        ...
        self.device = FuncGen(adress)
    
    def set_func1(self, value):
        """Set the function shape of the output
        ...
        self.device.ch1.set_function(value)
    
    def set_func2(self, value):
        """Set the function shape of the output
        ...
        self.device.ch2.set_function(value)
        
    def set_ampl1(self, value):
        
        try:
            if value > 0 and self.outp1() == 'OFF':
                self.set_outp1('ON')
        except ValueError:
            value = 0
            self.set_outp1('OFF')
            
        self.device.ch1.set_amplitude(value)
    
    def set_ampl2(self, value):
        
        try:
            if value > 0 and self.outp2() == 'OFF':
                self.set_outp2('ON')
        except ValueError:
            value = 0
            self.set_outp2('OFF')
    ...
    def set_offset1(self, value):
        ...
        self.device.ch1.set_offset(value)
    
    def set_offset2(self, value):
        
        self.device.ch2.set_offset(value)
        
    def set_outp1(self, value):
        
        self.device.ch1.set_output(value)
    
    def set_outp2(self, value):
        
        self.device.ch2.set_output(value)
    
    def set_custom_waveform1(self, waveform, memory_num=5, verify=True):
        
        self.device.set_custom_waveform(waveform, memory_num, verify=True)
        self.set_func1(f'USER{memory_num}')
    
    def set_custom_waveform2(self, waveform, memory_num=5, verify=True):
        
        self.device.set_custom_waveform(waveform, memory_num, verify=True)
        self.set_func2(f'USER{memory_num}')
    
    def ampl1(self):
        
        settings = self.device.ch1.get_settings()
        ans = settings['amplitude'][0]
        return ans
